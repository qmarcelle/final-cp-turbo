...
This monorepo follows centralized global configuration standards defined in `global-config.json`. All applications and packages inherit from these shared rules to ensure consistency across the entire codebase.

### Key Standards:
- **ESLint Configuration**: Shared via `@portals/eslint-config`
- **TypeScript**: Strict mode with shared configs
- **Prettier**: Consistent code formatting
- **Security**: Centralized headers and policies
...
3.  **Configure Corporate Proxy (BCBST Only):**
    ```bash
    # For BCBST developers behind corporate firewall
    ...
    > **Note**: This configures npm, pnpm, git, and environment variables for the BCBST corporate proxy (`webproxy.bcbst.com:80/443`). Skip this step if you're not on the BCBST network.
