...
export const navigationItemVariants = cva('nav-item', {
  variants: {
    variant: {
...
const useNavigationContext = () => {
  ...
  if (!context) {
    throw new Error(
      'Navigation compound components must be used within Navigation'
    )
  }
  return context
}
  ...
  ({ className, variant, position, ...props }, ref) => {
    ...
    return (
      <NavigationContext.Provider
        value={{ isOpen, setIsOpen, activeItem, setActiveItem, isMobile }}
      >
        <nav
          ref={ref}
          role="navigation"
...
const NavigationContent = React.forwardRef<
  HTMLDivElement,
  NavigationContentProps & HTMLMotionProps<'div'>
>(({ className, ...props }, ref) => {
    ...
    <NavigationMenuPrimitive.Item ref={ref} className={className} {...props}>
      ...
      >
        {React.Children.map(children, child => {
          if (
            React.isValidElement<NavigationDropdownItemProps>(child) &&
            child.type === NavigationDropdownItem
          ) {
