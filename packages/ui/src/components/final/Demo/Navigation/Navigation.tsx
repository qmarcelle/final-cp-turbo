...
const navContainerVariants = cva('bg-white', {
  variants: {
    variant: {
 ...
 */
export const NavigationBrand = React.forwardRef<
  HTMLDivElement,
  React.HTMLAttributes<HTMLDivElement>
>(({ className, ...props }, ref) => {
  return (
...
>(({ className, children, label, id, ...props }, ref) => {
  ...
  return (
    <li
      ref={dropdownRef}
      className={cn('relative', isMobile && 'w-full', className)}
      {...props}
    >
      <button
        type="button"
...
>(({ href, isCurrent, className, children, ...props }, ref) => {
  ...
  return (
    <li
      ...
      className={cn(
        'text-sm',
        isCurrent && 'font-medium text-tertiary-gray-1',
        className
      )}
      {...props}
    >
