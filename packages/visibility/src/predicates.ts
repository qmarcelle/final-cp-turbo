...
export function isUserInRole(userContext: any, role: string): boolean {
  console.log("isUserInRole called with context:", userContext, "and role:", role);
...
export function hasCoverageType(memberContext: any, productType: string): boolean {
  console.log("hasCoverageType called with context:", memberContext, "and productType:", productType);
  if (!memberContext || !Array.isArray(memberContext.coverageTypes)) {
    return false;
  }
  // throw new Error('Not implemented: hasCoverageType');
  return memberContext.coverageTypes.some((c: any) => c.productType === productType);
}
