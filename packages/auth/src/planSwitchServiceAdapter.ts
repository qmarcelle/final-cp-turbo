export const planSwitchServiceAdapter = {
  async handleSignIn(user: any): Promise<void> {
    console.log('planSwitchServiceAdapter.handleSignIn called for user:', user.id);
  ...
  async fetchMemberForPlan(userId: string, planId: string): Promise<any> {
    console.log(`planSwitchServiceAdapter.fetchMemberForPlan called for user: ${userId}, plan: ${planId}`);
