...
export const authOptions: NextAuthOptions = {
  providers: [
    CredentialsProvider({
      ...
      async authorize(credentials: Record<string, string> | undefined): Promise<AuthorizeResponse | null> {
        if (!credentials || !credentials.username) return null;

        const userIsValid = await verifyCredentials(credentials);
        if (!userIsValid) {
          console.error("Invalid primary credentials for:", credentials.username);
        ...
        if (mfaDevices && mfaDevices.length > 0) {
          if (!credentials.mfaToken || !mapMfaDevices(mfaDevices, credentials.mfaToken)) {
        ...
        return { id: mockUserId, name: credentials.username, email: 'placeholder@example.com' };
      }
    }) // CredentialsProvider is already a Provider, no need to cast `as Provider`
  ],
  callbacks: {
    async jwt({ token, user, account }: { token: JWT; user?: NextAuthUser | AuthorizeResponse; account?: Account | null; }): Promise<JWT> {
      if (user && user.id) {
        ...
        if (account) { // Typically for OAuth providers
        ...
        if (token.refreshToken && typeof token.expiresAt === 'number' && Date.now() > token.expiresAt) {
          try {
            ...
            token.expiresAt = refreshed.expires_at ? refreshed.expires_at * 1000 : undefined;
  ...
  events: {
    async signIn({ user, account, profile, isNewUser }: { user: NextAuthUser; account?: Account | null; profile?: Profile; isNewUser?: boolean; }) {
      if (user && user.id) {
